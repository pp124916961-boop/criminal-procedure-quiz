...
def grp(r):
 ...
 if any(k in t for k in ['行政處分之無效','重大明顯','程序補正','轉換','自始不生效力','效力繼續存在','送達','公告日']):return '效力與瑕疵'
...
def law(r):
 ...
 if any(k in z for k in ['重大明顯','行政處分之無效','自始不生效力']):return '第111條、第110條','行政處分原則自送達或通知發生效力；具有第111條重大明顯等法定無效事由者，自始不生效力。'
 if any(k in z for k in ['補正','轉換']):return '第114條至第116條','程序或方式瑕疵是否得補正、以及違法行政處分能否轉換，須依第114條至第116條分別判斷。'
