...
DST=Path('constitution-rights-50-round2')
...
if not qfile.exists():
    raise RuntimeError('round2 questions.json not found')
...
title_replacements={
    '刑法 → 行政處分 50 題歷屆原題':'憲法 → 基本原則＋自由權利 第二回 50 題歷屆原題',
    '行政處分 50 題歷屆原題':'憲法 → 基本原則＋自由權利 第二回 50 題歷屆原題',
    '刑法 → 行政處分':'憲法｜基本原則＋自由權利｜第二回',
    '刑法總則｜違法性':'憲法｜基本原則＋自由權利｜第二回',
    '刑法總則 違法性':'憲法｜基本原則＋自由權利｜第二回',
    '違法性｜50題歷屆原題':'憲法 → 基本原則＋自由權利 第二回 50 題歷屆原題'
}
...
ns='constitution-rights-50-round2'
...
if 'QuizWrongHistory?.mount' not in html:
    ...
    mount_js='''const wrongHistory = window.QuizWrongHistory?.mount({quizId:"constitution-rights-50-round2", title:"憲法－基本原則＋自由權利第二回50題", questions:QUESTIONS, getState:()=>state, idsAreQuestionIds:false}) || {capture(){},newRound(){}};\nif(!window.QuizWrongHistory){ const warning=document.createElement("p"); warning.textContent="錯題紀錄功能未載入，請確認網路後重新整理。"; (document.getElementById("controlPanel")||document.getElementById("home")).appendChild(warning); }\n'''
...
summary={
 ...
 'question_count':50,
 'new_path':'constitution-rights-50-round2',
