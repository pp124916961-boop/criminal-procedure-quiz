...
for q in qs:
    z=q['question']+' '+q['options']['ABCD'.index(q['answer'])]
...
# Regression checks for the two fact-pattern variants found in manual review.
q41=[q for q in qs if '不得對裁處案件之實體決定聲明不服' in q['question'] or '單獨對扣留' in q['question']]
assert q41 and all(q['basis']=='第41條' for q in q41), q41
q35=[q for q in qs if '強制其到指定處所查證身分不服' in q['question']]
