...
SRC=Path('admin-procedure-92-134'); DST=Path('admin-disposition-situational'); DST.mkdir(parents=True,exist_ok=True)
...
html=html.replace('行政程序法第92～134條','行政程序法｜行政處分實境辨識強化').replace('行政處分','行政處分實境辨識',1)
...
loader='''<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>行政程序法｜行政處分實境辨識強化｜100題歷屆原題</title><script>(async()=>{const names=['p1.txt','p2.txt','p2b.txt','p3.txt','p4.txt','p5.txt','p6.txt'];const a=(await Promise.all(names.map(n=>fetch(n).then(r=>r.text())))).join('');const b=atob(a);const u=Uint8Array.from(b,c=>c.charCodeAt(0));const t=await new Response(new Blob([u]).stream().pipeThrough(new DecompressionStream('gzip'))).text();document.open();document.write(t);document.close()})();</script>'''
(DST/'index.html').write_text(loader,encoding='utf-8')
summary={'template':'admin-procedure-92-134','question_count':100,'program_reused':True,'source_position':'above_question','source_in_explanation':False,'stale_source_urls_removed':True,'parts':{n:len(p) for n,p in zip(parts,out)}}
