...
assert all(q.get('source_url','').startswith('https://') for q in newq)
assert all(('wwwq.moex.gov.tw' in q['source_url']) or ('se.cpu.edu.tw' in q['source_url']) for q in newq)
...
repls={
    '刑法 → 行政處分 50 題歷屆原題':'憲法 → 基本原則＋自由權利 進階 100 題歷屆原題',
    '刑法 → 行政處分':'憲法 → 基本原則＋自由權利',
    '行政處分 50 題歷屆原題':'憲法 → 基本原則＋自由權利 進階 100 題歷屆原題',
    '刑法總則｜違法性':'憲法｜基本原則＋自由權利',
    '刑法總則 違法性':'憲法｜基本原則＋自由權利',
    '違法性｜50題歷屆原題':'憲法 → 基本原則＋自由權利 進階 100 題歷屆原題',
    ...
    '三等考試如原科目為申論題，本版不擅自改造成選擇題。高階選擇題以司法官／律師第一試原題補充。正犯共犯、罪數、刑罰等超出本次範圍者不收。':
      '本版為進階題組，範圍限憲法基本原則與自由權利；以司法官／律師第一試、司法三等、警佐班及警大二技等較高難度歷屆原題為主。',
    '司法官／律師第一試':'司法官／律師一試',
    '司法特考':'司法三等',
    '答案與解析依原題答案及現行刑法整理。':'答案依原考試官方答案整理；每題原題來源直接連結考選部試題。',
    '刑法總則｜違法性｜50題歷屆原題互動測驗｜本機自動保存進度':'憲法｜基本原則＋自由權利｜進階100題歷屆原題互動測驗｜本機自動保存進度'
}
...
ns='constitution-rights-100-hard'
...
mount_repl='QuizWrongHistory?.mount({quizId:"constitution-rights-100-hard", title:"憲法－基本原則＋自由權利進階100題", questions:QUESTIONS, getState:()=>state, idsAreQuestionIds:false})'
if 'QuizWrongHistory?.mount' in html:
    ...
    mount_js='''const wrongHistory = window.QuizWrongHistory?.mount({quizId:"constitution-rights-100-hard", title:"憲法－基本原則＋自由權利進階100題", questions:QUESTIONS, getState:()=>state, idsAreQuestionIds:false}) || {capture(){},newRound(){}};\nif(!window.QuizWrongHistory){ const warning=document.createElement("p"); warning.textContent="錯題紀錄功能未載入，請確認網路後重新整理。"; (document.getElementById("controlPanel")||document.getElementById("home")).appendChild(warning); }\n'''
...
assert all(x.get('url','').startswith('https://') for x in check)
assert all(('wwwq.moex.gov.tw' in x['url']) or ('se.cpu.edu.tw' in x['url']) for x in check)
...
assert 'quizId:"constitution-rights-100-hard"' in browser_html
assert 'https://wwwq.moex.gov.tw/' in browser_html
assert '刑法 → 行政處分' not in browser_html

REV='20260924-constitution-rights-100-hard-v1'
loader=f'''<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>憲法｜基本原則＋自由權利｜進階100題歷屆原題</title><script>(async()=>{{const v='{REV}';const names={json.dumps(parts)};const a=(await Promise.all(names.map(n=>fetch(n+'?v='+v,{{cache:'no-store'}}).then(r=>r.text())))).join('');const b=atob(a);const u=Uint8Array.from(b,c=>c.charCodeAt(0));const t=await new Response(new Blob([u]).stream().pipeThrough(new DecompressionStream('gzip'))).text();document.open();document.write(t);document.close()}})();</script>'''
...
summary={
 ...
 'program_reused':True,
 'question_count':100,
 'new_path':'constitution-rights-100-hard',
 'source_position':'above_question',
 'official_source_links':True,
 'difficulty_mix':'司法官／律師一試82＋司法三等10＋警佐5＋警大二技3',
